...
public class TestContractorMasterModifyPage extends BaseTestclass {

	@Test
	public void ContractorMaster_Modify() throws InterruptedException {
		...
		Thread.sleep(1000);
		String conname = FWUtills.read_XL_Data(XL_DATA_PATH, "ContractorMaster_Modify", 1, 0);
		String concode = FWUtills.read_XL_Data(XL_DATA_PATH, "ContractorMaster_Modify", 1, 1);
		String offcode = FWUtills.read_XL_Data(XL_DATA_PATH, "ContractorMaster_Modify", 1, 2);
		String offname = FWUtills.read_XL_Data(XL_DATA_PATH, "ContractorMaster_Modify", 1, 3);
		...
		if (invisible) {
			...
			gmp.clickContractorMasterModify();
			ContractorMasterPage contrms = new ContractorMasterPage(driver);
			WebElement frame1 = contrms.iframe;
			driver.switchTo().frame(frame1);
			contrms.EditContrname(conname);
			contrms.EditContrcode(concode);
			contrms.EditSearch();
			Thread.sleep(1000);
			contrms.Editlink();
			/*
			 * Thread.sleep(2000); contrms.Submitclick(); Alert al =
			 * driver.switchTo().alert(); String alertMessage = al.getText();
